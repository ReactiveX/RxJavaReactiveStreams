...
abstract class ManagedSubscription<T> implements Subscription {
    ...
    private final ConcurrentLinkedQueue<T> onNextQueue = new ConcurrentLinkedQueue<T>();

    protected ManagedSubscription(Subscriber<? super T> subscriber) {
    ...
    protected void start() {
        ...
        Throwable subscribeError = error.get();
        ...
        if (subscribeError == null) {
            ...
            stopped.set(true);
            subscriber.onError(subscribeError);
        }
    }

    @SuppressWarnings("ThrowableResultOfMethodCallIgnored")
    private void nextEvent() {
        if (started.get() && inOnMethod.compareAndSet(false, true)) {
            try {
